...
describe Pandora::Services::ShopService do
  ...
  describe "#get_similar_shops" do
    ...
    it "should return shop's that all attibutes are similar with input" do
      expect(subject.get_similar_shops("new shop", "ZhuQue Street", '100.10', '100.10').count).to eq 2
      expect(subject.get_similar_shops("new shop", "ChangQing Street", '100.10', '100.10').count).to eq 2
